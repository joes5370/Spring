...
public class HomeController {
	...
	public String home(Locale locale, Model model) {
		DBCommon<Player> dbCommon = new DBCommon<Player>("c:\\tomcat\\gameDB.sqlite", "player");
	...
	public String out(Locale locale, Model model, HttpServletRequest request) {
		...
		delete.method2(idx);
		session.invalidate();  // 회원 탈퇴 혹은 로그아웃 이기 때문에 세션을 지워줘야한다.
		return "redirect:/";
	}
...
//		dbCommon.updateData(new Player(id, password, name), idx);
	...
	public String battle(Locale locale, Model model, HttpServletRequest request, @RequestParam("select1") int select1, @RequestParam("select2") int select2) {
		...
		model.addAttribute("random_number", rand.nextInt(999999999));
